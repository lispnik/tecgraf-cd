...
/* macOS has no fontconfig by default, and the fontconfig path below rewrites Helvetica,
   Courier and Times into the DejaVu family names, which are a Linux convention and are not
   installed here -- so it matched nothing and every caller was told "no font". That is not a
   cosmetic failure: cdSimulationFont then has no file to hand FreeType, tt_text->face stays
   NULL, and cdSimulationGetTextSize returns without writing width or height at all. Anything
   built on the simulation -- the IMAGERGB driver in particular -- reports every string as 0x0
   and any caller laying out from text metrics draws nothing.

   CoreText is the system's own answer to this question and needs no third-party library. */
int cdGetFontFileNameSystem(const char *type_face, int style, char* filename)
{
  CFStringRef family_name;
  CFMutableDictionaryRef attributes;
  CTFontDescriptorRef descriptor, matched;
  CFURLRef font_url = NULL;
  CTFontSymbolicTraits symbolic_traits = 0;
  int found = 0;

  if (!type_face || !filename)
    return 0;

  /* Map the generic names CD callers use to families macOS actually ships. */
  if (cdStrEqualNoCase(type_face, "Courier") ||
  ...
  family_name = CFStringCreateWithCString(NULL, type_face, kCFStringEncodingUTF8);
  if (!family_name)
    return 0;
  ...
  CFDictionaryAddValue(attributes, kCTFontFamilyNameAttribute, family_name);

  if (symbolic_traits)
  {
    CFMutableDictionaryRef traits = CFDictionaryCreateMutable(NULL, 0,
                                      &kCFTypeDictionaryKeyCallBacks,
                                      &kCFTypeDictionaryValueCallBacks);
    if (traits)
    {
      CFNumberRef traits_value = CFNumberCreate(NULL, kCFNumberSInt32Type, &symbolic_traits);
      if (traits_value)
      {
        CFDictionaryAddValue(traits, kCTFontSymbolicTrait, traits_value);
        CFDictionaryAddValue(attributes, kCTFontTraitsAttribute, traits);
        CFRelease(traits_value);
      }
      CFRelease(traits);
    }
  }
  ...
  if (descriptor)
  {
    /* Match on the family alone: asking CoreText to match the traits as well fails outright
       for a family that has no italic cut, where falling back to the regular face is far
       better than reporting no font. */
    CFSetRef mandatory = CFSetCreate(NULL, (const void**)&kCTFontFamilyNameAttribute, 1,
                                     &kCFTypeSetCallBacks);
    matched = CTFontDescriptorCreateMatchingFontDescriptor(descriptor, mandatory);
    if (mandatory)
      CFRelease(mandatory);

    if (matched)
    {
      font_url = (CFURLRef)CTFontDescriptorCopyAttribute(matched, kCTFontURLAttribute);
      CFRelease(matched);
    }
  ...
  if (font_url)
  {
    /* FT_New_Face reads .ttc collections and .otf as happily as .ttf, so whatever CoreText
       points at can be used directly. */
    if (CFURLGetFileSystemRepresentation(font_url, true, (UInt8*)filename, 10240))
      found = 1;
    CFRelease(font_url);
  }
